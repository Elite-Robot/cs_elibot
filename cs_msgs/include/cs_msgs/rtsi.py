...
class rtsi(object):
    ...
    def controller_version(self):
        cmd = Command.RTSI_GET_URCONTROL_VERSION
    ...
    def set_input_param_check(self, input_data, param:str, relevance:str, relevance_param, ran:list):
        if input_data.__dict__.get(param) == None:
            return False
        if relevance != None:
            if input_data.__dict__.get(relevance) == None:
                return False
            if relevance_param != None:
                if input_data.__dict__[relevance] != relevance_param:
                    return False
        if ran != None:
            if input_data.__dict__[param] < ran[0]:
                return False
            if input_data.__dict__[param] > ran[1]:
                return False
        return True
    ...
    def get_output_data(self, binary=False):
        ...
        if self.__output_config is None:
            raise RTSIException('Output configuration not initialized')
        if self.__conn_state != ConnectionState.STARTED:
            raise RTSIException('Cannot receive when RTSI synchronization is inactive')
        return self.__recv(Command.RTSI_DATA_PACKAGE, binary)

    def get_output_data_buffered(self, binary = False, buffer_limit = None):
        ...
        try:
            ...
            data = self.__recv_from_buffer(Command.RTSI_DATA_PACKAGE, binary)
            if data == None:
                raise e
        else:
            data = self.__recv_from_buffer(Command.RTSI_DATA_PACKAGE, binary)
    ...
    def __on_packet(self, cmd, payload):
        ...
        elif cmd == Command.RTSI_GET_URCONTROL_VERSION:
    ...
    def __unpack_urcontrol_version_package(self, payload):
        if len(payload) != 16:
            _log.error('RTSI_GET_URCONTROL_VERSION: Wrong payload size')
            return None
